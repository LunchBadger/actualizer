...
class ServerlessAPIDeployment extends Deployment {
  ...
  getInitContainersSpec () {
    let cmd = `ssh-keyscan -t rsa -H git.gitea >> ~/.ssh/known_hosts; `;
    cmd += `ssh-keygen -f ~/.ssh/id_rsa -t rsa -N ""; `;
    cmd += `curl -X POST http://git-api.default/users/${CUSTOMER_NAMESPACE}/${this.producer}/ssh -d '{"type":"sls-api", "publicKey":"'"$(cat ~/.ssh/id_rsa.pub)"'"}'  -H "Content-type: application/json"; `;
    return [{
      name: 'init',
