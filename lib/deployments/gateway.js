...
export class GatewayDeployment extends Deployment {
  constructor(project, models, id, producer, env, domain) {
    ...
    pipelineIDs.forEach(id => {
      ...
      if (proxyPolicy.length) {
        const conditionActionPairs = proxyPolicy[0].proxy;
        conditionActionPairs.forEach(pair => {
          if (pair.action.serviceEndpoint &&
              this.gatewayConfig.serviceEndpoints.hasOwnProperty(pair.action.serviceEndpoint)) {
            pair.action.serviceEndpoint =
              this.gatewayConfig.serviceEndpoints[pair.action.serviceEndpoint];
          }
        });
      }
