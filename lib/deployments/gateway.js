...
module.exports = function load (producer, env, project, models, _, domain, slsFunctions) {
  if (!project) return [];
...
class GatewayDeployment extends Deployment {
    ...
    modelKeys.forEach(id => {
      if (!this.gatewayConfig.serviceEndpoints.hasOwnProperty(id)) {
        this.gatewayConfig.serviceEndpoints[id] =
        {
          ...
          url: `http://workspace-${this.producer}-${this.env}.customer:3000/api/${this.models[id].http.path}`
        };
      }
    });
  ...
  getConfigFiles () {
    ...
    return {
      'gateway.config.yml': jsYaml.safeDump(this.gatewayConfig),
      'system.config.yml': jsYaml.safeDump(Object.assign({}, defaultSystemConfig, this.systemConfig))
