...
const GATEWAY_VERSION = process.env.GATEWAY_VERSION || 'master';

export default function load(producer, env, project, models, _, domain) {
  /*if (env === 'dev' || !project) {
    return [];
  }*/
...
export class GatewayDeployment extends Deployment {
  constructor(project, models, id, producer, env, domain) {
    ...
    pipelineIDs.forEach(id => {
      ...
      const byTarget = project.connections.byTarget[id];
      const bySource = project.connections.bySource[id];

      let proxyTo;
      if (!byTarget || !byTarget[0]) {
        proxyTo = undefined;
      } else if (byTarget[0].fromId && !byTarget[0].from) {
        const fromId = byTarget[0].fromId;
        const modelKeys = Object.keys(this.models);
        if (modelKeys.indexOf(fromId) > -1) {
          if (!this.gatewayConfig.serviceEndpoints.hasOwnProperty(fromId)) {
            this.gatewayConfig.serviceEndpoints[fromId] =
              {
      ...
      if (proxyTo) {
        if (proxyPolicy.length > 0) {
          if (proxyPolicy[0].proxy.length === 0) {
            proxyPolicy[0].proxy.push({action: {}});
          }
