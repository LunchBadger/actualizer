...
export default function load(producer, env, project) {
  /*if (env === 'dev' || !project) {
    return [];
  }*/
  ...
  for (const id of Object.keys(project.gateways)) {
    deployments.push(new GatewayDeployment(project, id, producer, env));
  }
  return deployments;
}

export class GatewayDeployment extends Deployment {
  constructor(project, id, producer, env) {
    ...
    this.gatewayConfig.http = { host: '*', port: 8080 };
    this.gatewayConfig.admin = { hostname: '0.0.0.0', port: 9876 };
  }
