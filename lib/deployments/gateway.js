...
module.exports = function load (producer, env, project, models, _, domain, slsFunctions, openAPISpec) {
  ...
  if (producer.indexOf('vmware')) {
    console.log('this is special user with enabled auto gateway configuration');
    deployments.push(new AutoGatewayDeployment({project, models, producer, env, domain, slsFunctions, autoGeneratedPaths}));
  }

  return deployments;
};
class AutoGatewayDeployment extends Deployment {
  constructor ({project, models, producer, env, domain, slsFunctions, autoGeneratedPaths}) {
    super();
    ...
    this.gatewayConfig = {
      ...
      apiEndpoints: {},
      pipelines: {},
      policies: ['cors', 'proxy', 'key-auth', 'jwt']
    };
    this.gatewayConfig.apiEndpoints['api'] = {
      host: '*',
      paths: autoGeneratedPaths
    };
    this.gatewayConfig.serviceEndpoints['backend'] = {
...
class GatewayDeployment extends Deployment {
  constructor (project, models, id, producer, env, domain, slsFunctions) {
    ...
    console.log('AAAA', this.dnsPrefix);
    this.producer = producer;
    this.env = env;
