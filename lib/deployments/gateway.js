...
const defaultSystemConfig = JSON.stringify({
  db: {
    redis: {
...
class GatewayDeployment extends Deployment {
  ...
  getConfigFiles () {
    return {
      ...
      'system.config.yml': this.gatewayConfig.system ? JSON.stringify(this.gatewayConfig.system, null, '  ') : defaultSystemConfig
    };
  };
  ...
  getContainerSpec () {
    return [{
      ...
      env: [
        ...
        {
          name: 'REDIS_EMULATE',
          value: process.env.REDIS_PASSWORD || 'false'
        },
        {
          name: 'EG_NAMESPACE',
