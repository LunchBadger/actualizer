...
class Actualizer {
  ...
  async reconcile () {
    ...
    // TODO: clean removed secrets
    await Promise.all(producers.map(producer => {
      return this._catchErrors(() => this.reconcileSecrets(producer));
    }));
  ...
  async reconcileSecrets (producer) {
    debug('Updating secret for ', producer.name);
    const secret = await this.configStore.getSSHSecret(producer);
    console.log('resolved sec', secret);

    // TBD there is no need for env right now, SSH key does not depend on env
    const locator = {
      app: 'workspace', producer: producer.name
    };
    this.deployer.ensureSecret(locator, '', {
      metadata: {
        namespace: this.deployer.kubeSettings.namespace,
        name: 'git-secret-' + producer.name,
        labels: locator
      },
      data: {
        privateKey: Buffer.from(secret.privateKey).toString('base64'),
        publicKey: Buffer.from(secret.publicKey).toString('base64')
      }
    });
  }
  ...
  async reconcileDeployments (producer) {
    ...
    for (const env of envs) {
      ...
      const project = await this.getProject(producer.name, env);
      let models = [];
      ...
      const deployments =
        await this.loadDeployments(producer.name, env, project, models);
