...
async function reconcileRepos(configStore, deployer) {
  ...
  let repos = await _catchErrors(() => configStore.getAllRepos());
  if (!repos) {
    return;
  }
...
async function getConfigFile(configStore, repoId, env) {
  ...
  if (!configStr) {
    throw new ConsistencyError(repoId, env, `file ${fileName} not found`);
  }
  return configStr;
}
...
function configToDeployments(configStr, repoId, env) {
  ...
  try {
    ...
    if (err instanceof SyntaxError) {
      throw new ConsistencyError(repoId, env, `bad JSON: ${err.message}`);
    }
    throw err;
  }
...
async function reconcileDeployments(configStore, deployer, repo) {
  ...
  for (const env of envs) {
    const wantedRev = repo.branches['env/' + env];
    const configStr = await getConfigFile(configStore, repo.id, env);
    const deployments = configToDeployments(configStr, repo.id, env);

    await deployer.updateEnvironment(repo.id, env, deployments, wantedRev);
  }
}
