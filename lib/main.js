...
const SLS_API_URL_TEMPLATE = process.env.SLS_API_URL_TEMPLATE ||
  'http://sls-api-$PRODUCER-$ENV.customer';
...
class Actualizer {
  ...
  async reconcileDeployments (producer) {
    ...
    for (const env of envs) {
      ...
      const deployments =
        await this.loadDeployments({ producer: producer.name, env, project, models, slsFunctions });
  ...
  async getSLS ({producerID, env}) {
    const slsURL = SLS_API_URL_TEMPLATE
      ...
      .replace('$ENV', env) + '/services';

    debug('SLS URL', slsURL);
    try {
  ...
  async loadDeployments ({producer, env, project, models, slsFunctions}) {
    const result = [];
    for (const loader of this.loaders) {
      const deployments = await loader(producer, env, project, models, this.configStore, CUSTOMER_DOMAIN, slsFunctions);
