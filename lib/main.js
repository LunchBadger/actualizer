...
class Actualizer {
  ...
  async reconcile () {
    ...
    try {
      ...
      for (const prod of runningProds) {
        if (!producers.some(x => x.name === prod)) {
          debug('BBB producer needs to be removed', prod);
          producers.push({
            name: prod,
            forceDelete: true
          });
  ...
  async reconcileDeployments (producer) {
    ...
    for (const env of envs) {
      ...
      // If there is no project it is ok, it happens for new user
      // If project.disabled is true, admin manually added the flag to disable pod creation
      // TODO replace project.disabled with some licence, expiration etc.
      if (producer.forceDelete) {
        deployments = [];
      } else if (!project || (project && !project.disabled)) {
        let models = [];
        let openAPISpec;
        ...
        debug('project disabled for: ', producer);
      }
