...
export class Actualizer {
  ...
  async reconcileDeployments(producer) {
    ...
    for (const env of envs) {
      ...
      const deployments = await this.loadDeployments(env, project);
  ...
  async getProject(producerId, env) {
    ...
    return new ProjectConfig(objs);
  }

  async loadDeployments(env, project) {
    let result = [];
    for (let loader of this.loaders) {
      let deployments = await loader(env, project, this.configStore);
