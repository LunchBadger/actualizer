const debug = require('debug')('actualizer:csclient');
...
module.exports = class ConfigStoreClient {
  ...
  async _request (url, json = true) {
    ...
    try {
      ...
      debug(`Error communicating with configstore: ${err.message}`);
      throw new CSCommsError('could not complete request. ' + err.message);
    }
    if (req.statusCode === 404) {
      debug('Error communicating with configstore: 404');
      return null;
    } else if (req.statusCode !== 200) {
      debug(`Error communicating with configstore: ${req.statusCode}`);
