...
class Deployer {
  ...
  async upsertDeployment (locator, deploymentConfig, rev) {
    ...
    const deploymentReq = deploymentJson(locator, containers, volumes, rev, annotations);
...
function deploymentJson (locator, containers, volumes, rev, annotations) {
  ...
  const deployment = {
    ...
    spec: {
      ...
      template: {
        metadata: {
          labels: Object.assign({}, {rev}, locator)
        },
        spec: {
          imagePullSecrets: [
            {
