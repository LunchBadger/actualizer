...
const debug = dbg('actualizer:kube');

import _ from 'lodash';
import KubeClient from 'cisco-kube-client';
...
export class Deployer {
  ...
  async upsertDeployment(locator, deploymentConfig, rev) {
    ...
    const deploymentReq = deploymentJson(locator, containers, configMapReq);

    const vhost = (deploymentConfig.getVHost() + '.' +
                   this.config.customerDomain);
    ...
    if (Array.isArray(ports) && ports.length > 0) {
      ...
      const ingressReq = ingressJson(locator, ports, vhost);
...
export function configMapJson(locator, rev, configFiles) {
  if (Object.keys(configFiles).length === 0) {
    return null;
  }

  let name = locatorToName(locator) + '-' + rev.substr(0, 10);
...
function deploymentJson(locator, containers, configMap) {
  let name = locatorToName(locator);
...
function ingressJson(locator, ports, vhost) {
  let name = locatorToName(locator);

  const paths = ports.map(portSpec => {
    return {
      path: `/${portSpec.name}`,
      backend: {
        serviceName: name,
